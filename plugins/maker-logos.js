...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command, name }) => {
  if (!effect) throw '*[!INFO!] ?HOW TO USE THIS COMMAND?*\n-◉ _#logo (effect) (text)_\n*EXAMPLE:*\n-◉ _#logo 3d-deep -sea-metal Mystic_\n\n*[!] WHEN I TELL YOU THAT YOU NEED A TEXT, USE IT SERIOUSLY:*\n-◉ _#logo (effect) (text1|text2)_\n*EXAMPLE:*\n -◉ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n*<LIST OF EFFECTS/>*\n\no ඬ📝 #logo ' + effects.map(v => v.title).join(' \no ඬ📝 #logo ')
  effect = effect.toLowerCase()
  if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*[!INFO!] THE EFFECT ${effect} IS NOT IN THE LIST OF EFFECTS*`
  let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
  if (text.includes(split)) text = text.split(split)
  text = Array.isArray(text) ? text : [text]
  let res = await textpro(effect, ...text)
  if (typeof res == 'number') throw res == -1 ? `*[!INFO!] THE EFFECT ${effect} IS NOT IN THE EFFECT LIST*` : `*[!INFO!] THE CORRECT USE OF THE COMMAND IS ${usedPrefix + command} ${effect} ${new Array (res).fill('text').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
  let result = await axios.get(res, {
  responseType: 'arraybuffer'
  })
  await conn.sendFile(m.chat, result.data, 'Error.jpg', `*TAKE YOUR CUSTOM IMAGE!!*\n*EFFECT: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['nulis']
